Hello,

This is an MVC .NET project which is mixed up by AngularJS , Bootstrap  and .NET Core.

The project composed from two project, the client side & the server side for web API.

This project created by visual studio 2017.

You can run it by pressing F5.

After pressing F5, it will take some time to build and load the project.

There are 2 pages:
	
	* The first page is : Search 
	* The second page, is : Details

first page is in the top menu.

The default page is Reposit/Index , it's the searching page.


Please let me know if you have any questions.

have a nice week.
Regards,
Rafael Millis